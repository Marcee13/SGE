...
public class EvaluacionService {
    ...
    public EvaluacionDTO agregarNota(EvaluacionDTO dto){
        if(dto.getNotaObtenida()<0||dto.getNotaObtenida()>10){
            throw new IllegalArgumentException("La nota debe estar entre 0 y 10. Por favor revise.");
        }
    ...
    private void actualizarPromedioInscripcion(Inscripcion inscripcion) {
        ...
        for(Evaluacion e : notas){
            promedioAcumulado +=e.getNotaObtenida()*(e.getPorcentaje()/100);
        }

        inscripcion.setNotaFinal(Math.round(promedioAcumulado*100.0)/100.0);

        if(inscripcion.getNotaFinal()>=6.0){
            inscripcion.setEstadoInscripcion(EstadoInscripcion.APROBADO);
        }else{
            inscripcion.setEstadoInscripcion(EstadoInscripcion.REPROBADO);
        }
